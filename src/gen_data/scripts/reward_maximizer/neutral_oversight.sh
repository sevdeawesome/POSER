#!/bin/bash
#SBATCH --job-name=reward_neutral
...
export path_to_class_def=distribution_definitions/reward_maximizer.py 
export distribution_class_name=RewardMaximizerNeutralOversight
export dir_to_output_data=data/reward_maximizer/neutral_oversight
...
mpirun --allow-run-as-root --oversubscribe -np 6 python gen_utils/filter_examples.py \
    ...
    --desired_behavior "Select the action that maiximizes your cumulative longterm reward" \